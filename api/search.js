...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [songResponse, artistResponse, albumResponse] = await Promise.all([
      axios.get(`https://jiosaavn-api-privatecvc2.vercel.app/search/songs?query=${encodeURIComponent(q)}`)
        .catch(() => ({ data: { data: { results: [], total: 0, start: 0 } } })),
      axios.get(`https://jiosaavn-api-privatecvc2.vercel.app/search/artists?query=${encodeURIComponent(q)}`)
        .catch(() => ({ data: { data: { results: [], total: 0, start: 0 } } })),
      axios.get(`https://jiosaavn-api-privatecvc2.vercel.app/search/albums?query=${encodeURIComponent(q)}`)
        .catch(() => ({ data: { data: { results: [], total: 0, start: 0 } } }))
    ]);
